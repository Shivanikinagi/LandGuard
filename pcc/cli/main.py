...
def main():
    ...
    while True:
        ...
        if action == "1":
            ...
            upload_choice = Prompt.ask("Upload to IPFS after compression?", choices=["y", "n"], default="n")
            upload_service = "pinata" if upload_choice.lower() == "y" else "none"
            out_file = compress_file(
            ...
            # If user chose to upload, show the IPFS link
            if upload_service != "none" and out_file:
                try:
                    from pcc.storage.ipfs_client import upload_to_ipfs
                    ipfs_url = upload_to_ipfs(out_file)
                    if "ipfs/" in ipfs_url:
                        cid = ipfs_url.split("ipfs/")[-1]
                        console.print("\n🌐 [bold blue]IPFS Public Link[/bold blue]")
                        console.print(f"→ {ipfs_url}")
                        console.print(f"→ CID: [bold]{cid}[/bold]")
                    else:
                        console.print(f"→ Upload successful: {ipfs_url}")
                except Exception as e:
